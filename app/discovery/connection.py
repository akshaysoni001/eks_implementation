...
def __sts_connection():
    aws_svc_account_path = config.AWS_SVC_ACCOUNT_PATH
...
def parse_credentials(STS_FLAG):
    ...
    if STS_FLAG:
        ...
        with open("/app/credentials/aws_role_account.json", "r") as file_obj:
            data = json.load(file_obj)
            return {
...
class AWS:
    ...
    def __init__(self,
        ...
        self._aws_svc_account_file = os.path.join(os.getcwd() + aws_svc_account_path, svc_account_filename)
